...
use rotelyx_quic::{VarInt, crypto::rustls::QuicClientConfig};

/// ALPN for our quic addr discovery
pub const ALPN_QUIC_ADDR_DISC: &[u8] = b"/rotelyx_transport-qad/0";
...
pub(crate) mod server {
    ...
    impl QuicServer {
        ...
        pub(crate) fn spawn(
            ...
            let mut server_config = rotelyx_quic::ServerConfig::with_crypto(Arc::new(server_config));
            ...
            let endpoint = rotelyx_quic::Endpoint::server(server_config, bind_addr)
    ...
    async fn handle_connection(
        incoming: rotelyx_quic::Incoming,
        metrics: Arc<Metrics>,
        ...
        match connection_err {
            rotelyx_quic::ConnectionError::ApplicationClosed(ApplicationClose { error_code, .. })
...
pub enum Error {
    ...
    Connect {
        #[error(std_err)]
        source: rotelyx_quic::ConnectError,
    },
    #[error(transparent)]
    Connection {
        #[error(std_err)]
        source: rotelyx_quic::ConnectionError,
    },
    NoObservedAddr,
}
...
pub struct QuicClient {
    ...
    /// A client config.
    client_config: rotelyx_quic::ClientConfig,
}

impl QuicClient {
    /// Create a new QuicClient to handle the client side of QUIC
    /// address discovery.
    pub fn new(ep: rotelyx_quic::Endpoint, mut client_config: rustls::ClientConfig) -> Self {
        // add QAD alpn
        ...
        // a rotelyx_quic client config
        let mut client_config = rotelyx_quic::ClientConfig::new(Arc::new(
            QuicClientConfig::try_from(client_config).expect("known ciphersuite"),
        ));

        // enable the receive side of address discovery
        let mut transport = rotelyx_quic_proto::TransportConfig::default();
    ...
    async fn get_addr_and_latency(
        ...
        let Some(mut observed_addr) = external_addresses.next().await else {
            rotelyx_error::bail!(Error::NoObservedAddr);
        };
    ...
    pub async fn create_conn(
        ...
        server_addr: SocketAddr,
        host: &str,
    ) -> Result<rotelyx_quic::Connection, Error> {
...
mod tests {
    ...
    use rotelyx_quic::crypto::rustls::QuicServerConfig;
    use tracing::{Instrument, debug, info, info_span};
    ...
    async fn quic_endpoint_basic() -> Result {
        ...
        let client_endpoint =
            rotelyx_quic::Endpoint::client(SocketAddr::new(host.into(), 0)).std_context("client")?;
    ...
    async fn test_qad_client_closes_unresponsive_fast() -> Result {
        // create a client-side endpoint
        let client_endpoint = rotelyx_quic::Endpoint::client(SocketAddr::new(Ipv4Addr::LOCALHOST.into(), 0))
            .std_context("client")?;
    ...
    async fn test_qad_connect_delayed() -> Result {
        ...
        let mut server_config = rotelyx_quic::ServerConfig::with_crypto(Arc::new(
        ...
        let server_task = tokio::spawn(
            async move {
                ...
                let server = rotelyx_quic::Endpoint::new(
                    Default::default(),
                    Some(server_config),
                    socket.into_std().unwrap(),
                    Arc::new(rotelyx_quic::TokioRuntime),
                )
