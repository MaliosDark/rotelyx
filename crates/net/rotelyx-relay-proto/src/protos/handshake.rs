...
#[cfg(feature = "server")]
use rotelyx_transport_base::Signature;
use rotelyx_transport_base::{PublicKey, SecretKey};
use rotelyx_error::{AnyError, anyerr, e, ensure, stack_error};
use rotelyx_future::{SinkExt, TryStreamExt};
#[cfg(feature = "server")]
use rand::CryptoRng;
use tracing::trace;
...
const DOMAIN_SEP_CHALLENGE: &str = "rotelyx_transport-relay handshake v1 challenge signature";

/// Domain separation label for [`KeyMaterialClientAuth`]'s use of [`ExportKeyingMaterial`]
#[cfg(not(wasm_browser))]
const DOMAIN_SEP_TLS_EXPORT_LABEL: &[u8] = b"rotelyx_transport-relay handshake v1";
...
pub(crate) enum VerificationError {
    ...
    SignatureInvalid {
        source: rotelyx_transport_base::SignatureError,
        message: Vec<u8>,
        signature: [u8; 64],
...
mod tests {
    use bytes::BytesMut;
    use rotelyx_transport_base::{PublicKey, SecretKey};
    use rotelyx_error::{AnyError, Result, StackResultExt, StdResultExt};
    use rotelyx_future::{Sink, SinkExt, Stream, TryStreamExt};
