...
use rotelyx_error::{AnyError, Result, StdResultExt, bail_any};
...
impl TryFrom<AccessConfig> for Arc<dyn rotelyx_relay_proto::server::DynAccessControl> {
    type Error = AnyError;

    fn try_from(cfg: AccessConfig) -> Result<Self> {
        match cfg {
            AccessConfig::Everyone => Ok(Arc::new(rotelyx_relay_proto::server::AllowAll)),
...
struct TlsConfig {
    ...
    ///
    /// Defaults to the `https_bind_addr` with the port set to [`rotelyx_relay_proto::defaults::DEFAULT_RELAY_QUIC_PORT`].
    ///
    /// If `https_bind_addr` is not set, defaults to `http_bind_addr` with the
    /// port set to [`rotelyx_relay_proto::defaults::DEFAULT_RELAY_QUIC_PORT`]
