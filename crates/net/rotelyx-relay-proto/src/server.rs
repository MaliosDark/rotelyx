//! A fully-fledged rotelyx_transport-relay server over HTTP or HTTPS.
//!
//! This module provides an API to run a full fledged rotelyx_transport-relay server.  It is primarily
//! used by the `rotelyx_transport-relay` binary in this crate.  It can be used to run a relay server in
...
use rotelyx_future::{StreamExt, task::AbortOnDropHandle};
...
impl ServerConfig {
    ...
    ///
    /// **Rotelyx addition.** The struct is `#[non_exhaustive]` and upstream ships
    /// no constructor, which makes it impossible to build from another crate —
    /// so the server cannot be run by anyone who did not vendor this code.
    ///
    /// Metrics are off. Telemetry in a privacy tool is an operational dataset
    /// about who connected and when, living in a second place with weaker
    /// access control than the relay itself.
    pub fn new(relay: Option<RelayConfig>, quic: Option<QuicConfig>) -> Self {
        Self {
            relay,
            quic,
            #[cfg(feature = "metrics")]
...
pub struct Server {
    ...
    metrics_server: Option<rotelyx_metrics::service::MetricsServer>,
}
...
impl Server {
    ...
    pub async fn spawn(config: ServerConfig) -> Result<Self, SpawnError> {
        ...
        let metrics_server = if let Some(addr) = config.metrics_addr {
            ...
            let mut registry = rotelyx_metrics::Registry::default();
            registry.register_all(&metrics);
            let server = rotelyx_metrics::service::MetricsServer::spawn(addr, Arc::new(registry))
                .await
...
async fn relay_supervisor(
    ...
    let mut quic_fut = match quic_server {
        Some(ref mut server) => rotelyx_future::Either::Left(server.task_handle()),
        None => rotelyx_future::Either::Right(rotelyx_future::future::pending()),
    };
...
mod tests {
    ...
    use rotelyx_discovery::dns::DnsResolver;
