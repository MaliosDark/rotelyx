//! Streams used in the server-side implementation of rotelyx_transport relays.
...
use rotelyx_future::{FutureExt, Sink, Stream, ready, time};
use tokio::{
...
pub(crate) struct RateLimited<S> {
    ...
    ///
    /// Uses `tokio::sync::watch` and not `rotelyx_watcher` because in tokio's watch channel,
    /// checking if a value changed is a cheap atomic version compare, whereas `rotelyx_watcher`
