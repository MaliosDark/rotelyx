//! Embeds rotelyx_transport-relay into an [`axum::Router`].
//!
...
//! as expected by rotelyx_transport-relay's protocol handler, and then runs the
//! handshake plus client registration directly.
//!
//! [`BytesStreamSink`]: rotelyx_relay_proto::protos::streams::BytesStreamSink
...
use bytes::Bytes;
use rotelyx_transport_base::{RelayUrl, SecretKey};
...
use rotelyx_error::{AnyError, Result, StdResultExt};
use rotelyx_future::{Sink, Stream, task::AbortOnDropHandle};
...
/// Bridges axum's [`WebSocket`] to rotelyx_transport-relay's [`BytesStreamSink`].
