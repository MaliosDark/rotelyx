...
//! use rotelyx_watcher::{Watchable, Watcher as _};
//!
...
impl<W: Watcher + Unpin> rotelyx_future::Stream for Stream<W>
where
    W::Value: Unpin,
{
    type Item = W::Value;
...
mod tests {
    ...
    use rotelyx_future::{future::poll_once, StreamExt};
    use rand::{rng, RngExt};
    use tokio::{
        task::JoinSet,
    ...
    fn test_initialized_always_resolves() {
        ...
        let test_case = || {
            ...
            let thread = thread::spawn(move || rotelyx_future::future::block_on(watch.initialized()));
