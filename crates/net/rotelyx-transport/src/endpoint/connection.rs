//! The [`Connection`] wraps a `rotelyx_quic::Connection`.
//!
...
use rotelyx_transport_base::{EndpointId, RelayUrl};
use rotelyx_error::{e, stack_error};
use rotelyx_future::{TryFutureExt, future::Boxed as BoxFuture, time::Duration};
use rotelyx_quic::WeakConnectionHandle as NoqWeakConnectionHandle;
...
pub enum IncomingAddr {
    ...
    Custom(rotelyx_transport_base::CustomAddr),
}

impl From<IncomingAddr> for rotelyx_transport_base::TransportAddr {
...
pub struct Accept<'a> {
    #[pin]
    #[debug("rotelyx_quic::Accept")]
    pub(crate) inner: rotelyx_quic::Accept<'a>,
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        ...
        match this.inner.poll(cx) {
            ...
            Poll::Ready(Some(inner)) => {
                ...
                event!(
                    target: "rotelyx_transport::_events::conn::incoming",
                    tracing::Level::DEBUG,
...
fn alpn_from_noq_conn(conn: &rotelyx_quic::Connection) -> Option<Vec<u8>> {
    let data = conn.handshake_data()?;
    match data.downcast::<rotelyx_quic::crypto::rustls::HandshakeData>() {
        Ok(data) => data.protocol,
        Err(_) => None,
    }
}

async fn alpn_from_noq_connecting(conn: &mut rotelyx_quic::Connecting) -> Result<Vec<u8>, AlpnError> {
...
/// Converts a `rotelyx_quic::Connection` to a `Connection`.
///
...
> {
    ...
    event!(
        target: "rotelyx_transport::_events::conn::connected",
        tracing::Level::DEBUG,
...
fn static_info_from_conn(conn: &rotelyx_quic::Connection) -> Result<StaticInfo, AuthenticationError> {
...
fn remote_id_from_noq_conn(conn: &rotelyx_quic::Connection) -> Result<EndpointId, RemoteEndpointIdError> {
...
impl Connecting {
    ...
    ///
    /// [`RecvStream::is_0rtt`]: rotelyx_quic::RecvStream::is_0rtt
    #[allow(clippy::result_large_err)]
...
impl Accepting {
    pub(crate) fn new(inner: rotelyx_quic::Connecting, ep: Endpoint) -> Self {
        Self {
            inner,
            ep,
            register_with_socket: None,
...
impl<T: ConnectionState> Connection<T> {
    ...
    ///
    /// [`Session`]: rotelyx_quic_proto::crypto::Session
...
impl Connection<IncomingZeroRtt> {
    ...
    /// doesn't use the right TLS authentication, which usually every rotelyx_transport endpoint
    /// uses and requires.
    ///
    /// Thus, those errors should only occur if someone connects to you with a
    /// modified rotelyx_transport endpoint or with a plain QUIC client.
...
mod tests {
    ...
    use rotelyx_transport_base::{EndpointAddr, SecretKey};
    use rotelyx_relay_proto::tls::CaTlsConfig;
