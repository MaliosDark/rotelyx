//! Exporting and encapsulating structs from rotelyx_quic
//!
//! Co-locates all rotelyx_transport-rotelyx_quic exports.
//!
//! There are some structs that we use in particular ways, where we would like
//! to limit or expand how those structs are used in rotelyx_transport. By encapsulating them
//! we can ensure the functionality needed to make rotelyx_transport work.

#[cfg(feature = "qlog")]
use std::path::Path;
use std::{sync::Arc, time::Duration};

/// `rotelyx_quic` types that are used in the public rotelyx_transport API.
// Each type is notated with the rotelyx_transport type or rotelyx_quic type that uses it.
pub use rotelyx_quic::{
    AcceptBi,             // rotelyx_transport::endpoint::Connection
    AcceptUni,            // rotelyx_transport::endpoint::Connection
    AckFrequencyConfig,   // rotelyx_transport::endpoint::quic::QuicTransportConfig
    Closed,               // rotelyx_transport::endpoint::WeakConnectionHandle
    ClosedStream,         // rotelyx_transport::protocol::AcceptError, rotelyx_quic::RecvStream, rotelyx_quic::SendStream
    ConnectionError,      // rotelyx_transport::endpoint::ConnectError
    ConnectionStats,      // rotelyx_transport::endpoint::Connection
    Dir,                  // rotelyx_quic::StreamId
    IdleTimeout,          // rotelyx_transport::endpoint::quic::QuicTransportConfig
    MtuDiscoveryConfig,   // rotelyx_transport::endpoint::quic::QuicTransportConfig
    OpenBi,               // rotelyx_transport::endpoint::Connection
    OpenUni,              // rotelyx_transport::endpoint::Connection
    PathStats,            // rotelyx_transport::socket::remote_map::remote_state::PathInfo
    ReadDatagram,         // rotelyx_transport::endpoint::Connection
    ReadError,            // rotelyx_quic::RecvStream
    ReadExactError,       // rotelyx_quic::RecvStream
    ReadToEndError,       // rotelyx_quic::RecvStream
    RecvStream,           // rotelyx_quic::AcceptBi, rotelyx_quic::AcceptUni, rotelyx_quic::OpenBi, rotelyx_quic::OpenUni
    ResetError,           // rotelyx_quic::RecvStream
    SendDatagram,         // rotelyx_transport::endpoint::Connection
    SendDatagramError,    // rotelyx_transport::endpoint::Connection
    SendStream,           // rotelyx_quic::AcceptBi, rotelyx_quic::OpenUni
    Side,                 // rotelyx_transport::endpoint::Connection, rotelyx_quic::StreamId,
    StoppedError,         // rotelyx_quic::SendStream
    StreamId,             // rotelyx_quic::RecvStream
    UnorderedRecvStream,  // rotelyx_quic::RecvStream
    VarInt,               // various
    VarIntBoundsExceeded, // rotelyx_quic::VarInt, rotelyx_quic::IdleTimeout
    WriteError,           // rotelyx_quic::SendStream
};
#[cfg(feature = "qlog")]
pub use rotelyx_quic::{QlogConfig, QlogFactory, QlogFileFactory};
/// `rotelyx_quic_proto` types that are used in the public rotelyx_transport API.
// Each type is notated with the rotelyx_transport type or rotelyx_quic type that uses it.
pub use rotelyx_quic_proto::{
    ApplicationClose,                 // rotelyx_quic::ConnectionError
    Chunk,                            // rotelyx_quic::RecvStream
    ConnectError as QuicConnectError, // rotelyx_transport::endpoint::ConnectWithOptsError
    ConnectionClose,                  // rotelyx_quic::ConnectionError
    DecryptedInitial,                 // rotelyx_transport::endpoint::connection::Incoming
    FrameStats,                       // rotelyx_quic::ConnectionStats
    FrameType,                        // rotelyx_quic_proto::TransportError
    IncomingAlpns,                    // rotelyx_transport::endpoint::DecryptedInitial
    PathId,                           // rotelyx_quic_proto::crypto::PacketKey
    RttEstimator,                     // rotelyx_quic_proto::congestion::Controller
    TimeSource,                       // rotelyx_transport::endpoint::quic::ServerConfig
    TokenLog,                         // rotelyx_quic::ValidationTokenConfig
    TokenReuseError,                  // rotelyx_quic::TokenLog
    TransportError,                   // rotelyx_quic::ConnectionError
    TransportErrorCode,               // rotelyx_quic_proto::TransportError
    UdpStats,                         // rotelyx_quic::ConnectionStats
    ValidationTokenConfig,            // rotelyx_transport::endpoint::quic::::ServerConfig
    congestion::{
        Controller,        // rotelyx_transport::endpoint::Connection
        ControllerFactory, // rotelyx_transport::endpoint::quic::QuicTransportConfig
        ControllerMetrics, // rotelyx_quic_proto::congestion::Controller
    },
    crypto::{
        CryptoError,               // rotelyx_quic_proto::crypto::CryptoError, rotelyx_quic_proto::crypto::PacketKey
        ExportKeyingMaterialError, // rotelyx_transport::endpoint::Connection
        HandshakeTokenKey,         // rotelyx_transport::endpoint::quic::ServerConfig
        HeaderKey,                 // rotelyx_quic_proto::crypto::Keys
        Keys,                      // rotelyx_quic_proto::crypto::Session
        PacketKey,                 // rotelyx_quic_proto::crypto::Keys
        UnsupportedVersion,        // rotelyx_quic_proto::ConnectError
    },
    transport_parameters::TransportParameters, // rotelyx_quic_proto::crypot::ServerConfig
};
use tracing::warn;
...
#[derive(Debug, Clone)]
pub struct QuicTransportConfigBuilder(rotelyx_quic::TransportConfig);
...
///
/// In rotelyx_transport, the config has some specific default values that make rotelyx_transport's holepunching work
...
///
/// use rotelyx_transport::endpoint::QuicTransportConfig;
///
...
/// ```
#[derive(Debug, Clone)]
pub struct QuicTransportConfig(Arc<rotelyx_quic::TransportConfig>);
...
impl QuicTransportConfig {
    pub(crate) fn to_inner_arc(&self) -> Arc<rotelyx_quic::TransportConfig> {
        self.0.clone()
    }
}
...
impl QuicTransportConfigBuilder {
    ...
    fn new() -> Self {
        let mut cfg = rotelyx_quic::TransportConfig::default();
    ...
    /// # use rotelyx_transport::endpoint::{QuicTransportConfig, VarInt, VarIntBoundsExceeded};
    ...
    /// # Example
    /// ```
    /// # use rotelyx_transport::endpoint::QuicTransportConfig; use rotelyx_quic_proto::congestion; use std::sync::Arc;
    ...
    /// by all network interface drivers or packet inspection tools. `rotelyx_quic-udp` will attempt to
...
pub struct ServerConfigBuilder {
    inner: rotelyx_quic::ServerConfig,
...
// Note: used in `rotelyx_transport::endpoint::connection::Incoming::accept_with`
// This is new-typed since `rotelyx_quic::ServerConfig` takes a `TransportConfig`, which we new-type as a `QuicTransportConfig`
#[derive(Debug, Clone)]
pub struct ServerConfig(Arc<rotelyx_quic::ServerConfig>);

impl ServerConfig {
    pub(crate) fn to_inner_arc(&self) -> Arc<rotelyx_quic::ServerConfig> {
        self.0.clone()
    }
}
...
impl ServerConfigBuilder {
    ...
    pub(crate) fn new(inner: rotelyx_quic::ServerConfig, transport: QuicTransportConfig) -> Self {
    ...
    ///
    /// Defaults to [`rotelyx_quic::StdSystemTime`], which simply calls [`SystemTime::now()`](std::time::SystemTime::now).
    ///
