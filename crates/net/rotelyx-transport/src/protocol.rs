...
/// # {
/// # use std::sync::Arc;
/// # use rotelyx_error::StdResultExt;
...
//
// We are not using [`rotelyx_future::boxed::BoxFuture] because we don't need a `'static` bound
// on these futures.
...
impl Router {
    ...
    pub async fn shutdown(&self) -> Result<(), rotelyx_future::task::JoinError> {
        if self.is_shutdown() {
            return Ok(());
        }
...
mod tests {
    ...
    const ECHO_ALPN: &[u8] = b"/rotelyx_transport/echo/1";
    ...
    async fn test_limiter_hook() -> Result {
        ...
        impl EndpointHooks for LimitHook {
            async fn before_connect<'a>(
                &'a self,
                _remote_addr: &'a rotelyx_transport_base::EndpointAddr,
                alpn: &'a [u8],
    ...
    async fn test_graceful_shutdown() -> Result {
        ...
        const TEST_ALPN: &[u8] = b"/rotelyx_transport/test/1";
