...
use bytes::Bytes;
use rotelyx_transport_base::{CustomAddr, EndpointId, TransportAddr};
...
pub struct TestTransport {
    id: EndpointId,
    id_watchable: rotelyx_watcher::Watchable<Vec<CustomAddr>>,
    network: TestNetwork,
}
...
///
/// ```ignore
/// use rotelyx_transport::test_utils::custom_transport::TestNetwork;
///
/// let network = TestNetwork::new();
...
impl TestNetwork {
    ...
    pub fn create_transport(&self, id: EndpointId) -> io::Result<Arc<TestTransport>> {
        ...
        Ok(Arc::new(TestTransport {
            id_watchable: rotelyx_watcher::Watchable::new(vec![id_custom]),
            network: self.clone(),
            id,
        }))
    }
...
impl AddressLookup for TestAddrLookup {
    ...
    fn resolve(
    ...
    ) -> Option<rotelyx_future::stream::Boxed<Result<Item, crate::address_lookup::Error>>> {
        if self
            .network
            .inner
            .lock()
        ...
        {
            Some(Box::pin(rotelyx_future::stream::once(Ok(Item::new(
                EndpointInfo {
                    endpoint_id,
