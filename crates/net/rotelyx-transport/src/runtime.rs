...
impl rotelyx_quic::Runtime for Runtime {
    #[cfg(not(wasm_browser))]
    fn new_timer(&self, i: std::time::Instant) -> Pin<Box<dyn rotelyx_quic::AsyncTimer>> {
        rotelyx_quic::TokioRuntime.new_timer(i)
    }

    #[cfg(wasm_browser)]
    fn new_timer(&self, deadline: rotelyx_future::time::Instant) -> Pin<Box<dyn rotelyx_quic::AsyncTimer>> {
        Box::pin(web::Timer(rotelyx_future::time::sleep_until(deadline)))
    }
    ...
    // We're not actually using this function in rotelyx_transport
    #[cfg(not(wasm_browser))]
    fn wrap_udp_socket(
        &self,
        t: std::net::UdpSocket,
    ) -> std::io::Result<Box<dyn rotelyx_quic::AsyncUdpSocket>> {
        rotelyx_quic::TokioRuntime.wrap_udp_socket(t)
    }
}
...
mod web {
    ...
    use rotelyx_future::time;

    #[derive(Debug)]
    pub(crate) struct Timer(pub(crate) time::Sleep);

    impl rotelyx_quic::AsyncTimer for Timer {
