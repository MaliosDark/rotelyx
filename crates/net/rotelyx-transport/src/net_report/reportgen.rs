...
#[cfg(not(wasm_browser))]
use rotelyx_discovery::dns::{DnsError, DnsResolver, StaggeredError};
#[cfg(not(wasm_browser))]
use rotelyx_relay_proto::quic::QuicClient;
...
#[cfg(wasm_browser)]
use rotelyx_future::future::Pending;
use rotelyx_future::{
    StreamExt as _,
...
#[cfg(not(wasm_browser))]
/// Retry delays, in milliseconds, for staggered DNS lookups.
///
/// Rotelyx: moved here from the deleted `address_lookup::dns` provider. It is a
/// resolver retry schedule for our own relay hostnames and has nothing to do
/// with identity publishing, which is what that module was removed for.
...
impl From<rotelyx_netwatch::netmon::State> for IfStateDetails {
    fn from(value: rotelyx_netwatch::netmon::State) -> Self {
        IfStateDetails {
...
pub(crate) struct QuicConfig {
    /// A QUIC Endpoint
    #[debug("rotelyx_quic::Endpoint")]
    pub(crate) ep: rotelyx_quic::Endpoint,
    /// A client config.
...
async fn run_https_probe(
    ...
    // https://github.com/n0-computer/rotelyx_transport/issues/2901
    #[cfg(not(wasm_browser))]
    let mut builder = reqwest_client_builder(tls_config, dns_resolver.clone());
    #[cfg(wasm_browser)]
...
mod tests {
    ...
    async fn test_qad_probe_v4() -> Result {
        ...
        let client_config = rotelyx_relay_proto::tls::make_dangerous_client_config();
        let ep = rotelyx_quic::Endpoint::client(SocketAddr::new(Ipv4Addr::LOCALHOST.into(), 0)).anyerr()?;
        let client_addr = ep.local_addr().anyerr()?;

        let quic_client = rotelyx_relay_proto::quic::QuicClient::new(ep.clone(), client_config);
