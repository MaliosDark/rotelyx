...
//!
//! To connect to an rotelyx_transport endpoint a [`EndpointAddr`] is needed, which may contain a
...
//! information.  Each rotelyx_transport endpoint will automatically publish their own addressing
...
//! separate crates: [`rotelyx_transport-mdns-address-lookup`] and
//! [`rotelyx_transport-mainline-address-lookup`].
//!
//! [`rotelyx_transport-mdns-address-lookup`]: https://docs.rs/rotelyx_transport-mdns-address-lookup
//! [`rotelyx_transport-mainline-address-lookup`]: https://docs.rs/rotelyx_transport-mainline-address-lookup
//!
...
//! ```
//!
//! [`EndpointAddr`]: rotelyx_transport_base::EndpointAddr
...
use rotelyx_transport_base::{EndpointAddr, EndpointId};
pub use rotelyx_discovery::{ParseError, endpoint_info::AddrFilter};
use rotelyx_error::{AnyError, e, stack_error};
use rotelyx_future::{MergeBounded, Stream, boxed::BoxStream};
use tracing::debug;

pub use rotelyx_discovery::endpoint_info::{EndpointData, EndpointInfo, UserData};
use crate::{Endpoint, endpoint::EndpointError};

#[cfg(not(wasm_browser))]
// Rotelyx: the `dns` and `pkarr` address-lookup providers are deleted. Both
// publish this endpoint's public key to, or resolve peers from, infrastructure
// operated by somebody else — a disclosure of existence on every startup.
// Rendezvous happens at L3, sealed. `memory` is local-only and stays.
pub mod memory;
...
mod tests {
    ...
    use rotelyx_transport_base::{EndpointAddr, SecretKey, TransportAddr};
    use rotelyx_error::{AnyError, Result, StackResultExt};
    use rotelyx_future::{StreamExt, time};
        ...
        fn resolve(&self, endpoint_id: EndpointId) -> Option<BoxStream<Result<Item, Error>>> {
            ...
            let stream = match addr_info {
                Some((data, ts)) => {
                    ...
                    rotelyx_future::stream::once_future(fut).boxed()
                }
                None => rotelyx_future::stream::empty().boxed(),
            };
            Some(stream)
        }
    }
    ...
    impl AddressLookup for EmptyAddressLookup {
        ...
        fn resolve(&self, _endpoint_id: EndpointId) -> Option<BoxStream<Result<Item, Error>>> {
            Some(rotelyx_future::stream::empty().boxed())
        }
    }
    ...
    impl AddressLookup for FailingAddressLookup {
        ...
        fn resolve(&self, _endpoint_id: EndpointId) -> Option<BoxStream<Result<Item, Error>>> {
            ...
            Some(rotelyx_future::stream::once_future(fut).boxed())
        }
    }
    ...
    impl AddressLookup for HangingAddressLookup {
        ...
        fn resolve(&self, _endpoint_id: EndpointId) -> Option<BoxStream<Result<Item, Error>>> {
            Some(rotelyx_future::stream::pending().boxed())
        }
    }
    ...
    /// Regression test for https://github.com/n0-computer/rotelyx_transport/issues/4125
    ///
    ...
    async fn pending_resolve_survives_actor_idle_timeout() -> Result {
        ...
        // `rotelyx_transport::socket::remote_map::remote_state::ACTOR_MAX_IDLE_TIMEOUT`
...
// Rotelyx: the pkarr publish-and-resolve integration tests are removed along
// with the providers they exercised.
