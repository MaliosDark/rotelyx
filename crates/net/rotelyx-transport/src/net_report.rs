...
#[cfg(not(wasm_browser))]
use rotelyx_relay_proto::{RelayConfig, quic::QuicClient};
...
#[cfg(not(wasm_browser))]
use rotelyx_future::task;
use rotelyx_future::{
    StreamExt,
    task::AbortOnDropHandle,
    time::{self, Duration, Instant},
};
use rotelyx_watcher::{Watchable, Watcher};
...
enum QadProbeError {
    ...
    Quic { source: rotelyx_relay_proto::quic::Error },
...
impl QadConns {
    ...
    fn watch_v4(&self) -> impl rotelyx_future::Stream<Item = Option<QadProbeReport>> + Unpin + use<> {
        let watcher = self.v4.as_ref().map(|(_url, conn)| conn.observer.watch());

        if let Some(watcher) = watcher {
            watcher.stream_updates_only().boxed()
        } else {
            rotelyx_future::stream::empty().boxed()
        }
    }

    fn watch_v6(&self) -> impl rotelyx_future::Stream<Item = Option<QadProbeReport>> + Unpin + use<> {
...
impl Client {
    ...
    pub(crate) fn new(
        ...
        let quic_client = opts
            .quic_config
            .map(|c| rotelyx_relay_proto::quic::QuicClient::new(c.ep, c.client_config));
    ...
    pub(crate) async fn get_report(
        ...
        if self.have_enough_reports(&if_state, do_full, num_relays, &report) {
            ...
            #[cfg(wasm_browser)]
            let mut qad_v4_stream = rotelyx_future::stream::empty::<Option<()>>();
            #[cfg(not(wasm_browser))]
            let mut qad_v6_stream = self.qad_conns.watch_v6();
            #[cfg(wasm_browser)]
            let mut qad_v6_stream = rotelyx_future::stream::empty::<Option<()>>();
...
async fn run_probe_v4(
    ...
    shutdown_token: CancellationToken,
) -> rotelyx_error::Result<(QadProbeReport, QadConn), QadProbeError> {
    use rotelyx_quic_proto::PathId;
...
mod test_utils {
    ...
    use rotelyx_relay_proto::{RelayConfig, RelayQuicConfig, server};
