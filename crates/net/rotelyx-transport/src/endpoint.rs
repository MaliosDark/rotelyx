//! The [`Endpoint`] allows establishing connections to other rotelyx_transport endpoints.
//!
//! The [`Endpoint`] is the main API interface to manage a local rotelyx_transport endpoint.  It allows
//! connecting to and accepting connections from other endpoints.  See the [module docs] for
//! more details on how rotelyx_transport connections work.
//!
//! The main items in this module are:
//!
//! - [`Endpoint`] to establish rotelyx_transport connections with other endpoints.
...
use rotelyx_transport_base::{EndpointAddr, EndpointId, RelayUrl, SecretKey, TransportAddr};
use rotelyx_relay_proto::{RelayConfig, RelayMap, tls::CaTlsConfig};
#[cfg(not(wasm_browser))]
use rotelyx_error::bail;
use rotelyx_error::{AnyError, e, ensure, stack_error};
use rotelyx_watcher::Watcher;
...
impl Builder {
    ...
    pub async fn bind(self) -> Result<Endpoint, BindError> {
        ...
        let static_config = StaticConfig {
            ...
            token_key,
            token_store: Arc::new(rotelyx_quic::TokenMemoryCache::default()),
        };
        ...
        debug!(
            ...
            transport_version = %env!("CARGO_PKG_VERSION"),
            "rotelyx_transport endpoint bound"
        );
    ...
    ///
    /// [`PublicKey`]: rotelyx_transport_base::PublicKey
    ...
    ///
    /// Relay servers are used to establish initial connection with another rotelyx_transport endpoint.
    ...
    /// Sets the trusted CA root certificates for non-rotelyx_transport TLS connections.
    ///
    ...
    /// rotelyx_transport relays, pkarr servers, or DNS-over-HTTPS resolvers.
    /// They don't need to be trusted for the integrity or authenticity of native
    /// rotelyx_transport connections, which rely on rotelyx_transport's own cryptographic authentication mechanisms.
    ...
    ///
    /// This includes
    /// - TLS for encryption and authentication of rotelyx_transport connections themselves, but also
    ...
    ///
    /// If either the `tls-ring` or `tls-aws-lc-rs` feature is set in rotelyx_transport, this function doesn't
    /// need to be called.
    ///
    ...
    /// remote endpoint.  By default rotelyx_transport uses a built-in selector that sorts paths by
...
/// Controls an rotelyx_transport endpoint, establishing connections with other endpoints.
///
/// This is the main API interface to create connections to, and accept connections from
/// other rotelyx_transport endpoints.  The connections are peer-to-peer and encrypted, a Relay server is
/// used to make the connections reliable.  See the [crate docs] for a more detailed
/// overview of rotelyx_transport.
///
/// It is recommended to only create a single instance per application.  This ensures all
/// the connections made share the same peer-to-peer connections to other rotelyx_transport endpoints,
...
/// [`rotelyx_discovery::install_android_jni_context`] for details (the function is also
/// exported as `rotelyx_transport::dns::install_android_jni_context`).
///
/// If no JNI context is installed, rotelyx_transport relies on panic unwinding to detect
...
/// [`rotelyx_discovery::install_android_jni_context`]: https://docs.rs/rotelyx_transport-dns/latest/rotelyx_discovery/fn.install_android_jni_context.html
...
impl Endpoint {
    ...
    pub async fn connect_with_opts(
        ...
        event!(
            target: "rotelyx_transport::_events::conn::connecting",
            tracing::Level::DEBUG,
        ...
        // Start connecting via rotelyx_quic. This will time out after 10 seconds if no reachable
    ...
    /// # #[cfg(with_crypto_provider)]
    /// # {
    /// # use rotelyx_transport::{Watcher, Endpoint, endpoint::presets};
    ...
    /// [`RelayUrl`]: crate::RelayUrl
    #[cfg(not(wasm_browser))]
    pub fn watch_addr(&self) -> impl rotelyx_watcher::Watcher<Value = EndpointAddr> + use<> {
    ...
    /// # #[cfg(with_crypto_provider)]
    /// # {
    /// use rotelyx_transport::{Endpoint, Watcher as _, endpoint::presets};
    ///
    ...
    /// that want to use the same trust anchors as rotelyx_transport does for verifying the
    /// validity of TLS certificates presented by external services.
    ///
    /// Note that this TLS config is unrelated to how rotelyx_transport validates the authenticity
    /// of rotelyx_transport connections itself.
    ///
    ...
    /// To do so, use the [`rotelyx_metrics::Registry`], add the endpoint metrics to the
    ...
    /// Furthermore, [`rotelyx_metrics::service`] provides functions to easily start services
    ...
    /// let metrics_server = rotelyx_metrics::service::MetricsServer::spawn(addr, registry.clone())
    ///     .await
    ...
    /// ```
    ///
    /// [`Registry`]: rotelyx_metrics::Registry
    /// [`Registry::register_all`]: rotelyx_metrics::Registry::register_all
    /// [`Registry::sub_registry_with_label`]: rotelyx_metrics::Registry::sub_registry_with_label
    /// [`Registry::sub_registry_with_prefix`]: rotelyx_metrics::Registry::sub_registry_with_prefix
    /// [`encode_openmetrics_to_string`]: rotelyx_metrics::MetricsSource::encode_openmetrics_to_string
    /// [`MetricsGroup`]: rotelyx_metrics::MetricsGroup
    /// [`MetricsGroupSet`]: rotelyx_metrics::MetricsGroupSet
    #[cfg(feature = "metrics")]
    ...
    ///
    /// On many systems rotelyx_transport is able to detect network changes by itself, however
    ...
    /// function allows for notifying rotelyx_transport of any potential network changes like
    /// this.
    ///
    /// Even when the network did not change, or rotelyx_transport was already able to detect
...
impl EndpointClosed {
    ...
    pub async fn run_until<F: Future>(self, fut: F) -> Option<F::Output> {
        rotelyx_future::future::or(async { Some(fut.await) }, async {
            self.await;
            None
        })
        .await
...
pub enum RelayMode {
    ...
    Disabled,
    // Rotelyx: `Default` and `Staging` are deleted. Both resolved to relay
    // servers run by a third party, which is what this project exists not to
    // do. `Custom` is the only way to name a relay.
...
impl RelayMode {
    ...
    ///
    /// ```
    /// # fn main() -> rotelyx_error::Result<()> {
    /// # use rotelyx_transport::RelayMode;
    /// RelayMode::custom([
...
// Rotelyx: `force_staging_infra` and `default_relay_mode` are deleted. They read
// `IROH_FORCE_STAGING_RELAYS` from the environment and returned a relay mode
// pointing at third-party servers. Infrastructure that can be repointed from
// outside the process is a configuration-injection vector, and there is no
// default mode left for it to select.
...
mod tests {
    ...
    use rotelyx_transport_base::{EndpointAddr, EndpointId, RelayUrl, SecretKey, TransportAddr};
    use rotelyx_discovery::endpoint_info::UserData;
    use rotelyx_relay_proto::{RelayConfig, RelayQuicConfig, server::Access, tls::CaTlsConfig};
    use rotelyx_error::{AnyError as Error, Result, StdResultExt};
    use rotelyx_future::{BufferedStreamExt, StreamExt, future::now_or_never, stream, time};
    use n0_tracing_test::traced_test;
    use rotelyx_watcher::Watcher;
    use rotelyx_quic::PathStats;
    ...
    const TEST_ALPN: &[u8] = b"n0/rotelyx_transport/test";
    ...
    async fn endpoint_connect_close() -> Result {
        ...
        let server = tokio::spawn(
            async move {
                ...
                assert_eq!(
                    res.unwrap_err(),
                    rotelyx_quic::ReadToEndError::Read(rotelyx_quic::ReadError::ConnectionLost(
        ...
        let (server, client) = tokio::time::timeout(
            ...
            rotelyx_future::future::zip(server, client),
        )
        .await
        .anyerr()?;
        server.anyerr()??;
        ...
        let task = tokio::spawn({
            ...
            async move {
                let Some(conn) = server.accept().await else {
                    rotelyx_error::bail_any!("Expected an incoming connection");
                };
    ...
    async fn alpn_connection_test(
        ...
        let server_task = tokio::spawn({
            ...
            async move {
                ...
                let conn = incoming.await.anyerr()?;
                conn.close(0u32.into(), b"bye!");
                rotelyx_error::Ok(conn.alpn().to_vec())
            }
        });
    ...
    async fn same_endpoint_id_relay() -> Result {
        ...
        let expected_log_line = format!(
            "ep2:endpoint{{id={}}}:relay-actor:active-relay{{url={relay_url}}}:connected: rotelyx_transport::_events::relay::connected",
            ep2.id().fmt_short()
        );
    ...
    async fn test_closed_endpoint_behaviour() -> Result {
        ...
        let relay_config = rotelyx_relay_proto::RelayConfig::from(crate::RelayUrl::from("https://relay.example.invalid".parse::<url::Url>().expect("static url")));
        assert!(
    ...
    async fn test_endpoint_relay_auth_token() -> Result {
        ...
        impl rotelyx_relay_proto::server::AccessControl for TokenAccess {
            async fn on_connect(&self, request: &rotelyx_relay_proto::server::ClientRequest) -> Access {
