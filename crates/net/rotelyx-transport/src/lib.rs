...
//!
//! rotelyx_transport is a library to establish direct connectivity between peers. It exposes an
...
//!
//! An rotelyx_transport endpoint is created and controlled by the [`Endpoint`], e.g. connecting to
//! another endpoint:
//!
//! ```no_run
...
//!
//! An rotelyx_transport connection between two rotelyx_transport endpoints is usually established with the help
...
//!
//! If one of the rotelyx_transport endpoints can be reached directly, connectivity can also be
...
//! QUIC there is no client, server or server TLS key and certificate chain.  Instead each rotelyx_transport endpoint has a
//! unique [`SecretKey`] used to authenticate and encrypt the connection.  When an rotelyx_transport
//! endpoint connects, it uses the corresponding [`PublicKey`] to ensure the connection is only
//! established with the intended peer.
//!
//! Since the [`PublicKey`] is also used to identify the rotelyx_transport endpoint it is also known as
...
//! # Relay Servers
//!
//! Relay servers exist to ensure all rotelyx_transport endpoints are always reachable.  They accept
//! **encrypted** traffic for rotelyx_transport endpoints which are connected to them, forwarding it to
//! the correct destination based on the [`EndpointId`] only.  Since endpoints only send encrypted
//! traffic, the Relay servers can not decode any traffic for other rotelyx_transport endpoints and only
//! forward it.
//!
...
//!
//! Additionally to providing reliable connectivity between rotelyx_transport endpoints, Relay servers
...
//!
//! An rotelyx_transport endpoint is managed using the [`Endpoint`] and this is used to create or accept
//! connections to other endpoints.  To establish a connection to an rotelyx_transport endpoint you need to
...
//! [`EndpointId`] to connect to an rotelyx_transport endpoint can be an obstacle.  To address this, the
...
#![cfg_attr(rotelyx_docsrs, feature(doc_cfg))]
...
/// Path selection.
///
/// **Rotelyx modification.** Upstream keeps this module private, which makes
/// [`path_selection::PathSelector`] impossible to implement outside the crate
/// even though the endpoint builder accepts one. Rotelyx selects network paths by
/// metadata resistance rather than latency, so the trait and its context types
/// are exported here.
#[cfg(feature = "unstable-custom-transports")]
pub mod path_selection {
    pub use crate::socket::remote_map::{
        PathSelection, PathSelectionContext, PathSelectionData, PathSelector,
    };
    pub use crate::socket::transports::FourTuple;
}
...
pub use rotelyx_transport_base::{
...
pub use rotelyx_discovery::dns;
pub use rotelyx_relay_proto::{RelayConfig, RelayMap};
pub use rotelyx_watcher::Watcher;
