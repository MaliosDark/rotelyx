//! Utilities used in [`rotelyx_transport`](crate).
...
mod reqwest_dns_resolver {
    ...
    use rotelyx_discovery::dns::{DNS_TIMEOUT, DnsResolver};

    /// Implementation of [`reqwest::dns::Resolve`] for [`DnsResolver`].
    ///
    /// Wrapped in a newtype to not expose this in the public rotelyx_transport API.
