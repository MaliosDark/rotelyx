//! Implementation of encoding rotelyx_transport EndpointIds as domain names.
//!
...
//! We also add "rotelyx_transport" as a subdomain, although those 5 bytes might not be necessary.
...
pub(crate) fn encode(endpoint_id: EndpointId) -> String {
    format!(
        "{}.rotelyx_transport.invalid",
        BASE32_DNSSEC.encode(endpoint_id.as_bytes())
    )
}

pub(crate) fn decode(name: &str) -> Option<EndpointId> {
    let [base32_endpoint_id, "rotelyx_transport", "invalid"] = name.split(".").collect::<Vec<_>>()[..] else {
        return None;
    };
...
mod tests {
    ...
    fn test_snapshot() {
        ...
        assert_eq!(
            ...
            "7dl2ff6emqi2qol3l382krodedij45bn3nh479hqo14a32qpr8kg.rotelyx_transport.invalid",
        );
    }
}
