...
use rotelyx_relay_proto::protos::relay::Datagrams;
use rotelyx_future::{
    ready,
    task::{self, AbortOnDropHandle},
};
use rotelyx_watcher::Watcher as _;
use tokio::sync::mpsc;
...
pub(crate) struct RelayTransport {
    /// Queue to receive datagrams from relays for [`rotelyx_quic::AsyncUdpSocket::poll_recv`].
...
impl RelayTransport {
        ...
        for i in 0..bufs.len() {
            ...
            if buf_out.len() < dm.datagrams.contents.len() {
                ...
                warn!(
                    ...
                    "dropping received datagram: rotelyx_quic buffer too small"
                );
                break;
    ...
    pub(super) fn my_relay_status(&self) -> rotelyx_watcher::Direct<Option<RelayStatus>> {
        self.my_relay.watch()
    }
...
///
/// This includes the waker coordination required to support [`rotelyx_quic::UdpSender::poll_send`].
#[derive(Debug, Clone)]
...
fn datagrams_from_transmit(transmit: &Transmit<'_>) -> Datagrams {
    Datagrams {
        ecn: transmit.ecn.map(|ecn| match ecn {
            rotelyx_quic_udp::EcnCodepoint::Ect0 => rotelyx_quic_proto::EcnCodepoint::Ect0,
            rotelyx_quic_udp::EcnCodepoint::Ect1 => rotelyx_quic_proto::EcnCodepoint::Ect1,
            rotelyx_quic_udp::EcnCodepoint::Ce => rotelyx_quic_proto::EcnCodepoint::Ce,
        }),
