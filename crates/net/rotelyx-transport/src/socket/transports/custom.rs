...
///
/// A transport is a factory for custom endpoints. Whenever an rotelyx_transport endpoint is
...
pub trait CustomEndpoint: std::fmt::Debug + Send + Sync + 'static {
    ...
    ///
    /// The maximum length of the slices is [`rotelyx_quic_udp::BATCH_SIZE`].
...
pub trait CustomSender: std::fmt::Debug + Send + Sync + 'static {
    ...
    ///
    /// This will only be called from rotelyx_transport with addresses for which [CustomSender::is_valid_send_addr] returns true.
    ///
