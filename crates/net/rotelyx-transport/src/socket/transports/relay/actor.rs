...
//!
//! [`Client`]: rotelyx_relay_proto::client::Client
...
use rotelyx_netwatch::interfaces;
...
impl ActiveRelayActor {
    ...
    fn set_home_relay(&mut self, is_home: bool) {
        ...
        if self.is_home_relay != prev {
            event!(
                target: "rotelyx_transport::_events::relay::home_changed",
                Level::DEBUG,
                url = %self.url,
    ...
    async fn run_dialing(&mut self) -> Option<Result<rotelyx_relay_proto::client::Client, DialError>> {
    ...
    async fn run_connected(
        ...
        event!(
            target: "rotelyx_transport::_events::relay::connected",
            Level::DEBUG,
            url = %self.url,
            home_relay = self.is_home_relay,
        );
    ...
    fn handle_relay_msg(&mut self, msg: RelayToClientMsg, state: &mut ConnectedRelayState) {
        match msg {
            ...
            _ => unreachable!(
                "got unknown RelayToClientMsg but rotelyx_transport is released in sync with rotelyx_transport-relay"
            ),
        }
    }
    ...
    async fn run_sending<T>(
        ...
        client_stream: &mut rotelyx_relay_proto::client::ClientStream,
    ) -> Result<(), RelayConnectionError> {
...
impl HomeRelayWatch {
    ...
    pub(crate) fn watch(&self) -> rotelyx_watcher::Direct<Option<RelayStatus>> {
        self.inner.watch()
    }
}
...
impl RelayActor {
    ...
    async fn set_home_relay(&mut self, home_url: RelayUrl) {
        ...
        rotelyx_future::join_all(self.active_relays.iter().map(|(url, handle)| async move {
...
mod tests {
    ...
    use rotelyx_error::{AnyError as Error, Result, StackResultExt, StdResultExt};
    ...
    /// Starts an [`ActiveRelayActor`] as an "rotelyx_transport echo endpoint".
    ///
    /// This actor will connect to the relay server, pretending to be an rotelyx_transport endpoint, and echo
