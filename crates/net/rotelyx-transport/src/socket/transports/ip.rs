...
use rotelyx_netwatch::{UdpSender, UdpSocket};
...
impl IpTransport {
    pub(crate) fn bind(config: Config, metrics: Arc<SocketMetrics>) -> io::Result<Self> {
        ...
        debug!(?addr, "binding");
        let socket = rotelyx_netwatch::UdpSocket::bind_full(addr).inspect_err(|err| {
        ...
        match self.socket.poll_recv_noq(cx, bufs, metas) {
            ...
            Poll::Ready(Ok(n)) => {
                for i in 0..n {
                    ...
                    // The transport addresses are internal to rotelyx_transport and we always want those
    ...
    pub(super) fn local_addr_watch(&self) -> rotelyx_watcher::Direct<SocketAddr> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bind_sorting() -> rotelyx_error::Result {
