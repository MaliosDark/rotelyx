...
use rotelyx_error::StackResultExt;
use rotelyx_future::{
...
use rotelyx_quic::{Closed, PathStats, PathStatus, WeakConnectionHandle};
use rotelyx_quic_proto::{PathError, PathEvent as NoqPathEvent, PathId, n0_nat_traversal};
...
type PathEvents = MergeUnbounded<
    Pin<Box<dyn Stream<Item = (ConnId, Result<NoqPathEvent, rotelyx_quic::Lagged>)> + Send + Sync>>,
>;
...
type AddrEvents = MergeUnbounded<
    Pin<
        Box<
            dyn Stream<Item = (ConnId, Result<n0_nat_traversal::Event, rotelyx_quic::Lagged>)> + Send + Sync,
        >,
    >,
>;
...
struct State {
    ...
    local_direct_addrs: rotelyx_watcher::Direct<BTreeSet<DirectAddr>>,
...
impl RemoteStateActor {
    ...
    async fn run(
        ...
        rotelyx_future::pin!(idle_timeout);

        let check_connections = time::interval(UPGRADE_INTERVAL);
        rotelyx_future::pin!(check_connections);
        ...
        loop {
            ...
            rotelyx_future::pin!(scheduled_path_open);
            let scheduled_hp = match self.state.scheduled_holepunch {
                Some(when) => MaybeFuture::Some(time::sleep_until(when)),
                None => MaybeFuture::None,
            };
            rotelyx_future::pin!(scheduled_hp);
            ...
            tokio::select! {
                ...
                res = self.state.local_direct_addrs.updated() => {
                    if let Err(rotelyx_watcher::Disconnected) = res {
    ...
    fn handle_connection_close(&mut self, conn_id: ConnId, closed: Closed) {
        event!(
            target: "rotelyx_transport::_events::conn::closed",
            Level::DEBUG,
            %conn_id,
    ...
    #[instrument(skip(self))]
    fn handle_path_event(&mut self, conn_id: ConnId, event: Result<NoqPathEvent, rotelyx_quic::Lagged>) {
        ...
        match event {
            ...
            NoqPathEvent::Abandoned { id, reason, .. } => {
                ...
                event!(
                    target: "rotelyx_transport::_events::path::abandoned",
                    Level::DEBUG,
            ...
            _ => {
                // We expect to keep rotelyx_quic and rotelyx_transport in sync in all test setups, but in production it's totally possible
                // that rotelyx_transport itself is linked against a newer version of rotelyx_quic with additional events we don't yet
                // know how to handle.
                #[cfg(test)]
    ...
    fn select_path(&mut self) {
            ...
            && self.state.selected_path.as_ref() != Some(&addr)
        {
            ...
            event!(
                target: "rotelyx_transport::_events::path::selected",
                Level::DEBUG,
                remote = %self.state.endpoint_id.fmt_short(),
                network_path = %addr,
    ...
    /// Propagates a change of [`State::selected_path`] to rotelyx_quic.
    ///
        ...
        for (conn_id, conn_state) in self.connections.iter() {
            ...
            for (path_id, path_remote) in conn_state.paths.iter() {
                    ...
                    && conn_state.paths.values().filter(|a| a.is_ip()).count() > 1
                {
                    ...
                    match path.close() {
                        Err(rotelyx_quic_proto::ClosePathError::MultipathNotNegotiated) => {
                            error!("multipath not negotiated");
                        }
                        Err(rotelyx_quic_proto::ClosePathError::LastOpenPath) => {
                            error!("could not close last open path");
                        }
                        Err(rotelyx_quic_proto::ClosePathError::ClosedPath) => {
...
impl State {
    ...
    async fn handle_msg_send_datagram(
        ...
        if let Some(addr) = self.selected_path.as_ref() {
            ...
            // See https://github.com/n0-computer/rotelyx_transport/issues/4280.
    ...
    #[instrument(skip_all)]
    fn do_holepunching(&mut self, conn: rotelyx_quic::Connection) {
        ...
        match conn.initiate_nat_traversal_round() {
            Ok(remote_candidates) => {
                let remote_candidates = remote_candidates
                    ...
                    .collect();
                event!(
                    target: "rotelyx_transport::_events::qnt::init",
                    Level::DEBUG,
            ...
            Err(err) => {
                ...
                use rotelyx_quic_proto::n0_nat_traversal::Error;
                match err {
                    Error::Closed
    ...
    fn set_path_status(
        ...
        match path.set_status(status) {
            ...
            Ok(prev_status) if prev_status != status => {
                event!(
                    target: "rotelyx_transport::_events::path::set_status",
                    Level::DEBUG,
                    remote = %self.endpoint_id.fmt_short(),
                    %conn_id,
                    path_id=%path.id(),
    ...
    fn transport_tuple_for_path(&self, path: &rotelyx_quic::Path) -> Option<transports::FourTuple> {
...
/// [`DirectAddr`]s.
fn update_qnt_candidates(conn: &rotelyx_quic::Connection, direct_addrs: &BTreeSet<SocketAddr>) {
...
) -> impl Future<Output = rotelyx_error::Result<()>> + 'a {
...
pub(crate) enum RemoteStateMessage {
    ...
    ///
    /// The actor will downgrade the connection to a [`rotelyx_quic::WeakConnectionHandle`] as soon
    /// as it processes the message. It will keep hold of the weak handle until it closes,
    /// but only update to a strong [`rotelyx_quic::Connection`] for brief moments.
    ///
    /// The actor will actively manage paths on the connection and start holepunching as needed.
    #[debug("AddConnection({})", _0.stable_id())]
    AddConnection(rotelyx_quic::Connection, oneshot::Sender<PathStateReceiver>),
...
///
/// The wrapped value is the [`rotelyx_quic::Connection::stable_id`] value, and is thus only valid
...
//
// In production this borrows from a live connection and looks up stats from rotelyx_quic on
...
/// Poll a future once, like rotelyx_future::future::poll_once but sync.
...
///
/// This uses [`rotelyx_quic::Connection::on_closed`], which does not keep the connection alive
/// while awaiting the future.
struct OnClosed {
    conn_id: ConnId,
    inner: rotelyx_quic::OnClosed,
}

impl OnClosed {
    fn new(conn: &rotelyx_quic::Connection) -> Self {
        Self {
