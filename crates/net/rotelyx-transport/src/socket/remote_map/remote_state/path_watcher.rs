...
use rotelyx_transport_base::TransportAddr;
use rotelyx_future::{StreamExt, time::Duration};
use rotelyx_quic::WeakPathHandle;
...
impl PathData {
    /// Returns a strong [`rotelyx_quic::Path`].
    ///
    /// # Panics
    ///
    /// This may panic if the passed `rotelyx_quic::Connection` is not the one to which this path belongs.
    fn upgrade(&self, _conn: &rotelyx_quic::Connection) -> rotelyx_quic::Path {
        self.handle
            .upgrade()
...
impl PathStateSender {
    ...
    /// Records that a path was abandoned by `rotelyx_quic`.
    pub(super) fn record_abandoned(&self, id: PathId, conn: &rotelyx_quic::Connection) {
        let removed = {
            let mut state = self.shared.state.lock().expect("poisoned");
    ...
    ///
    /// [`WeakPathHandle`]: rotelyx_quic::WeakPathHandle
    pub(super) fn close(self, closed: rotelyx_quic::Closed) {
        let mut state = self.shared.state.lock().expect("poisoned");
        if !state.closed {
...
impl PathStateReceiver {
    ...
    pub(crate) fn get<'a>(&self, conn: &'a rotelyx_quic::Connection) -> PathList<'a> {
        PathList {
    ...
    /// closed.
    pub(crate) fn stream<'a>(&'a self, conn: &'a rotelyx_quic::Connection) -> PathListStream<'a> {
        PathListStream {
            shared: &self.shared,
            conn,
...
pub struct PathList<'conn> {
    snapshot: State,
    #[debug(skip)]
    conn: &'conn rotelyx_quic::Connection,
}
...
pub struct PathListIter<'a> {
    ...
    selected: Option<PathId>,
    conn: &'a rotelyx_quic::Connection,
}
...
pub struct Path<'a> {
    data: &'a PathData,
    is_selected: bool,
    /// Reference to a `rotelyx_quic::Connection` for safe upgrading via [`PathData::upgrade`]
    conn: &'a rotelyx_quic::Connection,
}
...
impl<'conn> Path<'conn> {
    ...
    /// the final statistics retained by `rotelyx_quic` for a path that closed
