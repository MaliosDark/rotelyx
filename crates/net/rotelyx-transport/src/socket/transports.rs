...
use bytes::Bytes;
use rotelyx_transport_base::{CustomAddr, EndpointId, RelayUrl, TransportAddr};
use rotelyx_relay_proto::RelayMap;
...
pub(crate) struct Transports {
    ...
    recv_infos: [RecvInfo; rotelyx_quic_udp::BATCH_SIZE],
    consecutive_total_recv_failures: usize,
}

/// Combined watcher type for all ip transports
type IpTransportsWatcher = rotelyx_watcher::Join<SocketAddr, rotelyx_watcher::Direct<SocketAddr>>;
/// Combined watcher type for all custom transports
type CustomTransportsWatcher =
    rotelyx_watcher::Join<Vec<CustomAddr>, rotelyx_watcher::Direct<Vec<CustomAddr>>>;
/// Combined watcher type for all relay transports
type RelayTransportsWatcher = rotelyx_watcher::Join<
    Option<(RelayUrl, EndpointId)>,
    rotelyx_watcher::Map<rotelyx_watcher::Direct<Option<RelayStatus>>, Option<(RelayUrl, EndpointId)>>,
>;

pub(super) type HomeRelayWatcher = rotelyx_watcher::Map<
    rotelyx_watcher::Join<Option<RelayStatus>, rotelyx_watcher::Direct<Option<RelayStatus>>>,
    Vec<RelayStatus>,
>;

#[cfg(not(wasm_browser))]
/// Combined watcher type for all transports, custom, relay and ip
pub(crate) type LocalAddrsWatch = rotelyx_watcher::Map<
    rotelyx_watcher::Tuple<
        rotelyx_watcher::Tuple<IpTransportsWatcher, CustomTransportsWatcher>,
...
pub(crate) type LocalAddrsWatch =
    rotelyx_watcher::Map<rotelyx_watcher::Tuple<CustomTransportsWatcher, RelayTransportsWatcher>, Vec<Addr>>;
...
impl Transports {
    ...
    pub(crate) fn poll_recv(
        ...
        assert!(bufs.len() <= rotelyx_quic_udp::BATCH_SIZE, "too many buffers");
        if sock.is_closed() {
            return Poll::Pending;
        }
    ...
    pub(super) fn home_relay_watch(&self) -> HomeRelayWatcher {
        rotelyx_watcher::Join::new(self.relay.iter().map(|t| t.my_relay_status()))
    ...
    pub(crate) fn local_addrs_watch(&self) -> LocalAddrsWatch {
        let ips = rotelyx_watcher::Join::new(self.ip.iter().map(|t| t.local_addr_watch()));
        let relays = rotelyx_watcher::Join::new(self.relay.iter().map(|t| t.local_addr_watch()));
        let custom = rotelyx_watcher::Join::new(self.custom.iter().map(|t| t.watch_local_addrs()));
    ...
    pub(crate) fn max_receive_segments(&self) -> NonZeroUsize {
        ...
        // that rotelyx_quic creates. Having buffers slightly bigger than necessary
...
mod tests {
    ...
    fn sample_ready_custom_transport_selection(
        ...
        let mut metas = [rotelyx_quic_udp::RecvMeta::default()];
        ...
        for _ in 0..polls {
            ...
            storage[0] = 0;
            metas[0] = rotelyx_quic_udp::RecvMeta::default();
        }
    ...
    impl CustomEndpoint for ReadyCustomEndpoint {
        fn watch_local_addrs(&self) -> rotelyx_watcher::Direct<Vec<CustomAddr>> {
...
pub struct Transmit<'a> {
    pub(crate) ecn: Option<rotelyx_quic_udp::EcnCodepoint>,
    /// Packet contents
    pub contents: &'a [u8],
...
impl From<&rotelyx_quic_udp::Transmit<'_>> for OwnedTransmit {
    fn from(source: &rotelyx_quic_udp::Transmit<'_>) -> Self {
        Self {
            ecn: source.ecn,
...
///
/// This carries the bits of [`rotelyx_quic_udp::RecvMeta`] that custom transports
...
pub enum LocalTransportAddr {
    ...
    Custom(Option<rotelyx_transport_base::CustomAddr>),
}

impl LocalTransportAddr {
    /// Converts a local address from rotelyx_quic into a [`LocalTransportAddr`].
    ///
    /// This also needs the `remote_addr`, because currently the meaning of the local_ip as returned
    /// from rotelyx_quic depends on the kind of network path, which we can gather from the remote address.
    ///
    ...
    ///   through [`RecvInfo`], which is passed as a mapped address to rotelyx_quic. So we convert it
    ...
    pub(super) fn from_noq_local_ip(
        ...
        match &remote_addr {
            ...
            // For IP transports, the local_ip as reported from rotelyx_quic is the interface IP (umapped), if known.
...
impl FourTuple {
    ...
    /// Returns the [`FourTuple] for a rotelyx_quic network path by looking up QUIC-mapped addresses.
    pub(super) fn from_noq(
        noq_four_tuple: rotelyx_quic::FourTuple,
    ...
    /// Returns the QUIC-mapped [`rotelyx_quic::FourTuple`] for this [`FourTuple`].
    pub(super) fn to_noq_four_tuple(
        &self,
        relay_mapped_addrs: &AddrMap<(RelayUrl, EndpointId), RelayMappedAddr>,
        custom_mapped_addrs: &AddrMap<CustomAddr, CustomMappedAddr>,
    ) -> rotelyx_quic::FourTuple {
        ...
        rotelyx_quic::FourTuple::new(remote, local)
    }
...
impl rotelyx_quic::AsyncUdpSocket for Transport {
    fn create_sender(&self) -> Pin<Box<dyn rotelyx_quic::UdpSender>> {
        Box::pin(Sender {
    ...
    fn poll_recv(
        ...
        cx: &mut Context,
        bufs: &mut [IoSliceMut<'_>],
        meta: &mut [rotelyx_quic_udp::RecvMeta],
    ) -> Poll<io::Result<usize>> {
...
impl Sender {
    /// Extracts the right [`Addr`] from the [`rotelyx_quic_udp::Transmit`].
    ///
    ...
    fn mapped_addr(&self, transmit: &rotelyx_quic_udp::Transmit) -> io::Result<MultipathMappedAddr> {
...
impl rotelyx_quic::UdpSender for Sender {
    fn poll_send(
        self: Pin<&mut Self>,
        noq_transmit: &rotelyx_quic_udp::Transmit,
        cx: &mut Context,
    ) -> Poll<io::Result<()>> {
