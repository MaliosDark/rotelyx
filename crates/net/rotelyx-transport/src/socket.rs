...
use rotelyx_relay_proto::{RelayConfig, RelayMap};
use mapped_addrs::MultipathMappedAddr;
use rotelyx_error::{AnyError, anyerr, bail, e, stack_error};
use rotelyx_future::{
    MaybeFuture,
    task::{self, AbortOnDropHandle},
    time::{self, Duration, Instant},
};
use rotelyx_watcher::{self, Watchable, Watcher};
use rotelyx_netwatch::netmon;
#[cfg(not(wasm_browser))]
use rotelyx_netwatch::{
...
/// rotelyx_transport 0.35 used 10s at the QUIC level; tailscale uses 45s at the WireGuard session
...
pub(crate) struct Options {
    ...
    /// TLS configuration for HTTPS and non-rotelyx_transport-QUIC connections.
    pub(crate) tls_config: rustls::ClientConfig,

    /// ServerConfig for the internal QUIC endpoint
    pub(crate) server_config: rotelyx_quic_proto::ServerConfig,
...
/// Inner state for an rotelyx_transport [`crate::Endpoint`].
///
...
pub(crate) struct EndpointInner {
    ...
    // rotelyx_quic endpoint
    endpoint: rotelyx_quic::Endpoint,
    // Runtime used by rotelyx_quic
    runtime: Arc<Runtime>,
    /// Static configuration for the endpoint.
    pub(crate) static_config: StaticConfig,
}
...
/// Configuration for a [`rotelyx_quic::Endpoint`] that cannot be changed at runtime.
#[derive(derive_more::Debug)]
...
impl StaticConfig {
    /// Create a [`rotelyx_quic_proto::ServerConfig`] with the specified ALPN protocols.
    pub(crate) fn create_server_config(
        &self,
        alpn_protocols: Vec<Vec<u8>>,
    ) -> rotelyx_quic_proto::ServerConfig {
        let mut quic_server_config = self.server_config.clone();
        quic_server_config.set_alpn_protocols(alpn_protocols);
        let mut inner =
            rotelyx_quic::ServerConfig::new(Arc::new(quic_server_config), self.token_key.clone());
        inner.transport_config(self.transport_config.to_inner_arc());
        inner
    }

    /// Create a [`rotelyx_quic_proto::ClientConfig`] with the specified ALPN protocols.
    pub(crate) fn create_client_config(
        &self,
        alpn_protocols: Vec<Vec<u8>>,
        transport_config: Arc<rotelyx_quic::TransportConfig>,
    ) -> rotelyx_quic_proto::ClientConfig {
        let mut quic_client_config = self.client_config.clone();
        quic_client_config.set_alpn_protocols(alpn_protocols);
        let mut inner = rotelyx_quic::ClientConfig::new(Arc::new(quic_client_config));
...
struct ShutdownState {
    ...
    /// Token that is cancelled once the [`rotelyx_quic::Endpoint`] is drained.
    ///
...
impl Socket {
    ...
    /// Whether the rotelyx_transport endpoint is closed and all its actors stopped.
    ...
    ///
    /// [`Watcher`]: rotelyx_watcher::Watcher
    /// [`Watcher::initialized`]: rotelyx_watcher::Watcher::initialized
    pub(crate) fn ip_addrs(&self) -> rotelyx_watcher::Direct<BTreeSet<DirectAddr>> {
    ...
    fn process_datagrams(
        ...
        metas: &mut [rotelyx_quic_udp::RecvMeta],
...
impl EndpointInner {
    ...
    pub(crate) async fn bind(opts: Options) -> Result<Self, BindError> {
        ...
        if let Some(v4_port) = transports.local_addrs().into_iter().find_map(|t| {
            ...
            // NOTE: we can end up with a zero port if `rotelyx_netwatch::UdpSocket::socket_addr` fails
        ...
        let mut endpoint_config =
            rotelyx_quic::EndpointConfig::new(Arc::new(Blake3HmacKey::new(&mut rand::rng())));
        // Setting this to false means that rotelyx_quic will ignore packets that have the QUIC fixed bit
        ...
        // through to rotelyx_quic. We set the first byte of the packet to zero, which makes rotelyx_quic ignore
        ...
        let endpoint = rotelyx_quic::Endpoint::new_with_abstract_socket(
            endpoint_config,
            Some(server_config),
        ...
        let net_report_config = {
            ...
            // If we would, the `rotelyx_quic::Endpoint` passed along will not have IP connectivity,
    ...
    /// Returns a reference to the underlying [`rotelyx_quic::Endpoint`].
    pub(crate) fn noq_endpoint(&self) -> &rotelyx_quic::Endpoint {
        &self.endpoint
    }

    /// Closes the rotelyx_transport endpoint.
    ///
    /// Only the first close does anything. Any later closes return nil.  Polling the socket
    /// ([`rotelyx_quic::AsyncUdpSocket::poll_recv`]) will return [`Poll::Pending`] indefinitely
    /// after this call.
    ///
    ...
    pub(crate) async fn close(&self) {
        ...
        // - removed: https://github.com/n0-computer/rotelyx_transport/pull/1753
        // - then added back in: https://github.com/n0-computer/rotelyx_transport/pull/2227/files#diff-ba27e40e2986a3919b20f6b412ad4fe63154af648610ea5d9ed0b5d5b0e2d780R573
        // - then removed again: https://github.com/n0-computer/rotelyx_transport/pull/3165
        ...
        // With the current rotelyx_quic API, this is the only way to ensure protocol code can use
        ...
        // Expects that the `rotelyx_quic::Endpoint` has been closed before this call,
    ...
    ///
    /// This should only be called in the `rotelyx_transport::Endpoint` `Drop` impl when the
    /// `rotelyx_transport::Endpoint` is dropped without first calling `Endpoint::close`.
    #[instrument(skip_all, parent = self.sock.span.clone())]
    ...
    /// the remote, thus there is no point in trying to connect over the rotelyx_quic endpoint.
    ///
...
struct Actor {
    ...
    /// [`crate::Endpoint`].
    endpoint: rotelyx_quic::Endpoint,
    /// Shared state between an awful lot of rotelyx_transport subsystems.
    ///
    ...
    local_interfaces_watcher: rotelyx_watcher::Direct<netmon::State>,
    ...
    async fn run(
        ...
        while !shutdown_token.is_cancelled() {
            ...
            let notify_quic_network_change = match &self.call_notify_quic_network_change {
                Some(pending) => {
                    MaybeFuture::Some(rotelyx_future::time::sleep_until(pending.next_check))
                }
                None => MaybeFuture::None,
            };
            rotelyx_future::pin!(notify_quic_network_change);
            ...
            tokio::select! {
                ...
                state = self.local_interfaces_watcher.updated() => {
                    ...
                    event!(
                        target: "rotelyx_transport::_events::link_change",
                        Level::DEBUG,
                        ?state,
                        is_major
                    );
    ...
    fn notify_quic_network_change(&mut self, is_major: bool) {
        ...
        impl NetworkChangeHint for Hint {
            fn is_path_recoverable(
                &self,
                _path_id: rotelyx_quic::PathId,
                network_path: rotelyx_quic_proto::FourTuple,
            ) -> bool {
...
impl DiscoveredDirectAddrs {
    ...
    fn update(&self, addrs: BTreeSet<DirectAddr>) -> bool {
        *self.updated_at.write().expect("poisoned") = Some(Instant::now());
        ...
        if updated {
            event!(
                target: "rotelyx_transport::_events::direct_addrs",
                Level::DEBUG,
...
/// A *direct address* on which an rotelyx_transport-endpoint might be contactable.
///
/// Direct addresses are UDP socket addresses on which an rotelyx_transport endpoint could potentially be
/// contacted.  These can come from various sources depending on the network topology of the
/// rotelyx_transport endpoint, see [`DirectAddrType`] for the several kinds of sources.
///
...
///
/// These are the various sources or origins from which an rotelyx_transport endpoint might have found a
/// possible [`DirectAddr`].
...
pub enum DirectAddrType {
    ...
    ///
    /// When possible an rotelyx_transport endpoint will perform QAD to discover which is the address
    ...
    ///
    /// When possible an rotelyx_transport endpoint will request a port mapping from the local router to
    /// get a publicly routable direct address.
    Portmapped,
    /// Hard NAT: QAD'ed IPv4 address + local fixed port.
    ///
    /// It is possible to configure rotelyx_transport to bound to a specific port and independently
    /// configure the router to forward this port to the rotelyx_transport endpoint.  This indicates a
...
mod tests {
    ...
    use rotelyx_transport_base::{EndpointAddr, EndpointId, TransportAddr};
    use rotelyx_relay_proto::tls::{CaTlsConfig, default_provider};
    use rotelyx_error::{Result, StackResultExt, StdResultExt};
    use rotelyx_future::{MergeBounded, StreamExt, time};
    ...
    /// Creates a new [`rotelyx_quic::Endpoint`] hooked up to a [`Socket`].
    ///
    ...
    async fn socket_connect(
        ep: rotelyx_quic::Endpoint,
        ep_secret_key: SecretKey,
        addr: EndpointIdMappedAddr,
        endpoint_id: EndpointId,
    ) -> Result<rotelyx_quic::Connection> {
        // Endpoint::connect sets this, do the same to have similar behaviour.
        let mut transport_config = rotelyx_quic::TransportConfig::default();
    ...
    async fn socket_connect_with_transport_config(
        ...
        let mut client_config = rotelyx_quic::ClientConfig::new(Arc::new(quic_client_config));
    ...
    async fn test_try_send_no_send_addr() {
        ...
        let accept_task = tokio::spawn({
            async fn accept(ep: rotelyx_quic::Endpoint) -> Result<()> {
    ...
    async fn test_try_send_no_udp_addr_or_relay_url() {
        ...
        // Set a low max_idle_timeout so rotelyx_quic gives up on this quickly and our test does
