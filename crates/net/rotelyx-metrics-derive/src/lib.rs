...
fn expand_iterable(input: &DeriveInput) -> Result<proc_macro2::TokenStream, Error> {
    ...
    let metric_arms = metrics.iter().enumerate().map(|(i, f)| {
        ...
        quote! {
            #i => Some(::rotelyx_metrics::MetricItem::new(#ident_str, #help, &self.#ident as &dyn ::rotelyx_metrics::Metric)),
        }
    });
    let family_arms = families.iter().enumerate().map(|(i, f)| {
        let (ident, ident_str, help) = (f.ident, &f.ident_str, &f.help);
        quote! {
            #i => Some(::rotelyx_metrics::FamilyItem::new(#ident_str, #help, &self.#ident as &dyn ::rotelyx_metrics::FamilyEncoder)),
        }
    });
    ...
    let family_impl = (family_count > 0).then(|| {
        quote! {
            ...
            fn family_field_ref(&self, n: usize) -> Option<::rotelyx_metrics::FamilyItem<'_>> {
                match n {
                    #(#family_arms)*
                    _ => None,
                }
    ...
    Ok(quote! {
        impl ::rotelyx_metrics::iterable::Iterable for #name {
            fn metric_field_count(&self) -> usize { #metric_count }
            fn metric_field_ref(&self, n: usize) -> Option<::rotelyx_metrics::MetricItem<'_>> {
                match n {
                    #(#metric_arms)*
                    _ => None,
                }
...
fn expand_metrics(input: &DeriveInput) -> Result<proc_macro2::TokenStream, Error> {
    ...
    Ok(quote! {
        impl ::rotelyx_metrics::MetricsGroup for #name {
...
fn expand_metrics_group_set(input: &DeriveInput) -> Result<proc_macro2::TokenStream, Error> {
    ...
    for field in fields {
        ...
        cloned.extend(quote! {
            self.#name.clone() as ::std::sync::Arc<dyn ::rotelyx_metrics::MetricsGroup>,
        });
        refs.extend(quote! {
            &*self.#name as &dyn ::rotelyx_metrics::MetricsGroup,
        });
    }

    Ok(quote! {
        impl ::rotelyx_metrics::MetricsGroupSet for #name {
            fn name(&self) -> &'static str {
                #name_str
            }

            fn groups_cloned(&self) -> impl ::std::iter::Iterator<Item = ::std::sync::Arc<dyn ::rotelyx_metrics::MetricsGroup>> {
                [#cloned].into_iter()
            }

            fn groups(&self) -> impl ::std::iter::Iterator<Item = &dyn ::rotelyx_metrics::MetricsGroup> {
                [#refs].into_iter()
            }
        }
    })
...
fn expand_encode_label_set(input: &DeriveInput) -> Result<proc_macro2::TokenStream, Error> {
    ...
    for field in fields.iter() {
        ...
        label_pairs.push(quote! {
            (#label_name, ::rotelyx_metrics::EncodeLabelValue::encode_label_value(&self.#ident))
        });
    }

    Ok(quote! {
        impl ::rotelyx_metrics::EncodeLabelSet for #name {
            fn encode_label_pairs(&self) -> ::std::vec::Vec<::rotelyx_metrics::LabelPair<'_>> {
                ::std::vec![#(#label_pairs),*]
            }
        }
    })
...
fn expand_encode_label_value(input: &DeriveInput) -> Result<proc_macro2::TokenStream, Error> {
    ...
    for variant in &data.variants {
        ...
        arms.push(quote! {
            Self::#ident => ::rotelyx_metrics::LabelValue::Str(::std::borrow::Cow::Borrowed(#label)),
        });
    }

    Ok(quote! {
        impl ::rotelyx_metrics::EncodeLabelValue for #name {
            fn encode_label_value(&self) -> ::rotelyx_metrics::LabelValue<'_> {
                match self {
                    #(#arms)*
                }
            }
