...
use rotelyx_error::{AnyError, StackError, StdResultExt, e, stack_error};
use rotelyx_future::{
...
/// Trait for DNS resolvers used in rotelyx_transport.
...
pub struct StaggeredError<E: rotelyx_error::StackError + 'static> {
    errors: Vec<E>,
}
...
/// The DNS resolver used throughout `rotelyx_transport`.
///
...
impl Inner {
        ...
        Fut: 'static + Send + Future<Output = Result<R, E>>,
    {
        loop {
            ...
            let timeout = rotelyx_future::time::sleep(timeout);
            tokio::pin!(timeout);
...
impl DnsResolver {
    ...
    // Rotelyx: the `lookup_endpoint_by_*` family is deleted. Those functions
    // resolved a peer's reachability from `_iroh` TXT records published to a
    // third party DNS zone, which is the discovery mechanism this project
    // removed. What remains resolves hostnames to IP addresses, which is what
    // reaching our own relays requires.
}
...
>(
    ...
    let mut calls = rotelyx_future::FuturesUnorderedBounded::new(delays_ms.len() + 1);
