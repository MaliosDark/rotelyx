...
//! reads `LinkProperties.getDnsServers()` through [`ndk_context`]. rotelyx_transport on
...
/// Exposes a JVM to rotelyx_transport so that we can read the system's DNS configuration.
///
...
/// via `JNI_OnLoad`:
///
/// *Note: `install_android_jni_context` is reexported from `rotelyx_transport`, so you can substitute
/// `rotelyx_discovery` for `rotelyx_transport` below.*
///
/// ```ignore
...
///     unsafe {
///         rotelyx_discovery::install_android_jni_context(vm, res);
///     }
