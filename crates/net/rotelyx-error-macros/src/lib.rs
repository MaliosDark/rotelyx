...
/// * **`add_meta`** adds a `rotelyx_error::Meta` field to the struct or each each enum variant. The `Meta`
...
/// expands to
/// ```ignore
/// #[derive(rotelyx_error::StackError)]
/// #[error(from_sources)]
/// enum MyError {
///     #[error("io failed")]
///     Io {
///         source: std::io::Error,
///         meta: rotelyx_error::Meta,
///     },
///     #[error("remote failed with {_0}")]
///     RemoteErrorCode(u32, #[error(meta)] rotelyx_error::Meta),
/// }
/// ```
...
fn modify_attrs(args: &StackErrAttrArgs, attrs: &mut Vec<Attribute>) -> Result<(), syn::Error> {
    ...
    if args.derive {
        attrs.insert(0, parse_quote!(#[derive(::rotelyx_error::StackError)]));
    }
...
fn add_meta_field(fields: &mut Fields) {
    ...
    match fields {
        Fields::Named(fields) => {
            let field: syn::Field = parse_quote! { #[doc = #doc] meta: ::rotelyx_error::Meta };
        ...
        Fields::Unnamed(fields) => {
            let field: syn::Field = parse_quote! { #[doc = #doc] #[error(meta)] ::rotelyx_error::Meta };
...
///   - `#[error(meta)]: Sets a field as the `meta` field for this error or variant. Must have type [`::rotelyx_error::Error`]
///
...
impl<'a> SourceField<'a> {
    fn expr_error_ref(&self, expr: proc_macro2::TokenStream) -> proc_macro2::TokenStream {
        match self.kind {
            SourceKind::Std => quote! { Some(::rotelyx_error::ErrorRef::std(#expr)) },
            SourceKind::Stack => quote! { Some(::rotelyx_error::ErrorRef::stack(#expr)) },
        }
    }
    ...
    fn expr_error_std(&self, expr: proc_macro2::TokenStream) -> proc_macro2::TokenStream {
        match self.kind {
            ...
            SourceKind::Stack => quote! { Some(::rotelyx_error::StackError::as_std(#expr)) },
        }
    }
}
        ...
        self.from.as_ref().map(|from_field| {
            ...
            let fields = self.fields.iter().map(|field| {
                if field.ident == from_field.ident {
                    ...
                    field.ident.init(quote!(::rotelyx_error::Meta::new()))
                } else {
...
fn generate_enum_impls(
    ...
    quote! {
        impl #impl_generics ::rotelyx_error::StackError for #enum_ident #ty_generics #where_clause {
            ...
            fn as_dyn(&self) -> &(dyn ::rotelyx_error::StackError) {
                self
            }
            ...
            fn meta(&self) -> Option<&::rotelyx_error::Meta> {
                match self {
                    #( #match_meta_arms, )*
                }
            }
            fn source(&self) -> Option<::rotelyx_error::ErrorRef<'_>> {
                match self {
                    #( #match_source_arms )*
                }
            }
        ...
        impl #impl_generics ::core::convert::From<#enum_ident #ty_generics> for ::rotelyx_error::AnyError #where_clause {
            fn from(value: #enum_ident #ty_generics) -> ::rotelyx_error::AnyError {
                ::rotelyx_error::AnyError::from_stack(value)
            }
        }

        impl #impl_generics ::std::fmt::Display for #enum_ident #ty_generics #where_clause {
            fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
                let sources = f.alternate().then_some(::rotelyx_error::SourceFormat::OneLine);
                write!(f, "{}", ::rotelyx_error::StackError::report(self).sources(sources))
            }
        }
        ...
        impl #impl_generics ::std::fmt::Debug for #enum_ident #ty_generics #where_clause {
            fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
                if f.alternate() {
                    ...
                    write!(f, "{}", ::rotelyx_error::StackError::report(self).full())?;
                }
                Ok(())
            }
        }
...
fn generate_struct_impl(
    ...
    let constructor = {
        ...
        let fields = info.fields.iter().map(|f| {
            if f.is_meta() {
                f.ident.init(quote!(::rotelyx_error::Meta::new()))
            } else {
                let ident = f.ident.as_ident();
                quote!(#ident)
            }
    ...
    quote! {
        ...
        impl #impl_generics ::rotelyx_error::StackError for #item_ident #ty_generics #where_clause {
            fn as_std(&self) -> &(dyn ::std::error::Error + ::std::marker::Send + ::std::marker::Sync + 'static) {
                self
            }
            fn into_std(self: ::std::boxed::Box<Self>) -> ::std::boxed::Box<dyn std::error::Error + ::std::marker::Send + ::std::marker::Sync> {
        ...
        impl #impl_generics ::core::convert::From<#item_ident #ty_generics> for ::rotelyx_error::AnyError #where_clause {
            fn from(value: #item_ident #ty_generics) -> ::rotelyx_error::AnyError {
                ::rotelyx_error::AnyError::from_stack(value)
            }
        }
