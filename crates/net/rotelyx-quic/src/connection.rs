...
impl Connecting {
    ...
    /// information. See [`rotelyx_quic_udp::RecvMeta::dst_ip`](udp::RecvMeta::dst_ip) for a list of
    /// supported platforms when using [`rotelyx_quic_udp`](udp) for I/O, which is the default.
    ///
            ...
            match event {
                ...
                Path(ref evt @ PathEvent::Abandoned { id, .. }) => {
                    if let Some(sender) = self.open_path.remove(&id) {
                        ...
                        // With the rotelyx_quic API, there is no way to application-close a not-yet-opened path, so
                ...
                _ => {
                    ...
                    // It's possible that rotelyx_quic is built against a newer rotelyx_quic-proto version.
                    // In that case, we need to ignore path events we can't handle yet.
                    // But for tests, we expect rotelyx_quic and rotelyx_quic-proto to be in sync, so we
