...
use crc::Crc;
use rotelyx_quic::{self, ConnectionError, ReadError, StoppedError, TransportConfig, WriteError};
use rand::{self, Rng};
...
fn connect_n_nodes_to_1_and_send_1mb_data() {
    ...
    let endpoint =
        rotelyx_quic::Endpoint::server(cfg, SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 0)).unwrap();
    ...
    for _ in 0..expected_messages {
        ...
        runtime.spawn(async move {
            if let Err(e) = task.await {
                use rotelyx_quic::ConnectionError::*;
                match e {
...
async fn read_from_peer(mut stream: rotelyx_quic::RecvStream) -> Result<(), ConnectionError> {
    let crc = Crc::<u32>::new(&crc::CRC_32_ISO_HDLC);
    match stream.read_to_end(1024 * 1024 * 5).await {
        ...
        Err(e) => {
            use ReadError::*;
            use rotelyx_quic::ReadToEndError::*;
            match e {
...
async fn write_to_peer(conn: rotelyx_quic::Connection, data: Vec<u8>) -> Result<(), WriteError> {
...
fn configure_connector(node_cert: CertificateDer<'static>) -> rotelyx_quic::ClientConfig {
    ...
    let mut peer_cfg = rotelyx_quic::ClientConfig::with_root_certificates(Arc::new(roots)).unwrap();
    peer_cfg.transport_config(Arc::new(transport_config));
    peer_cfg
}

/// Builds listener configuration along with its certificate.
fn configure_listener() -> (rotelyx_quic::ServerConfig, CertificateDer<'static>) {
    let (our_cert, our_priv_key) = gen_cert();
    let mut our_cfg =
        rotelyx_quic::ServerConfig::with_single_cert(vec![our_cert.clone()], our_priv_key.into()).unwrap();
