...
async fn run(options: Opt) -> Result<()> {
    let (certs, key) = if let (Some(key_path), Some(cert_path)) = (&options.key, &options.cert) {
        ...
        let dirs = directories_next::ProjectDirs::from("org", "rotelyx_quic", "rotelyx_quic-examples").unwrap();
    ...
    let mut server_config =
        rotelyx_quic::ServerConfig::with_crypto(Arc::new(QuicServerConfig::try_from(server_crypto)?));
    ...
    let endpoint = rotelyx_quic::Endpoint::server(server_config, options.listen)?;
...
async fn handle_connection(root: Arc<Path>, conn: rotelyx_quic::Incoming) -> Result<()> {
    let connection = conn.await?;
    let span = info_span!(
        "connection",
        remote = ?connection.path(rotelyx_quic::PathId::ZERO).and_then(|p| p.remote_address().ok()),
        protocol = %connection
            .handshake_data()
            .unwrap()
            .downcast::<rotelyx_quic::crypto::rustls::HandshakeData>().unwrap()
            .protocol
    ...
    async {
        ...
        loop {
            ...
            let stream = match stream {
                Err(rotelyx_quic::ConnectionError::ApplicationClosed { .. }) => {
...
async fn handle_request(
    root: Arc<Path>,
    (mut send, mut recv): (rotelyx_quic::SendStream, rotelyx_quic::RecvStream),
) -> Result<()> {
    let req = recv
        .read_to_end(64 * 1024)
        .await
