...
use rotelyx_quic::{Endpoint, TokioRuntime};
...
struct Context {
    server_config: rotelyx_quic::ServerConfig,
...
impl Context {
    fn new() -> Self {
        ...
        let mut server_config =
            rotelyx_quic::ServerConfig::with_single_cert(vec![cert.clone()], key.into()).unwrap();
        ...
        Self {
            server_config,
            client_config: rotelyx_quic::ClientConfig::with_root_certificates(Arc::new(roots)).unwrap(),
        }
    }
    ...
    ) -> (rotelyx_quic::Endpoint, rotelyx_quic::Connection, Runtime) {
        let runtime = rt();
        let endpoint = {
            let _guard = runtime.enter();
