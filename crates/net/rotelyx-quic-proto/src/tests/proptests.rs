...
// These TransportConfig constants are designed to match rotelyx_transport for now.
...
fn allowed_error(err: Option<ConnectionError>) -> bool {
    ...
    if !allowed {
        error!(
            ?err,
            "Got an error that's unexpected in rotelyx_quic <-> rotelyx_quic interaction"
        );
    }
    allowed
}
...
/// Regression test for the "invalid key" panic in `rotelyx_quic-proto::Endpoint::handle_event`.
///
...
/// A regression test that used to put rotelyx_quic into a state of sending PATH_CHALLENGE
...
/// > rotelyx_quic_proto::connection: 4704:
