...
/// behaviour when this is encountered. rotelyx_quic chooses to accept and then close the connection,
...
/// Regression test simulating a situation that would trigger an unreachable! in rotelyx_quic.
///
/// - rotelyx_quic expects that there always is a `ConnectionError` set at the end of draining
///   a connection.
/// - When the connection close is generated within rotelyx_quic-proto (or received remotely),
///   then this error in rotelyx_quic is populated via connection events.
/// - This test simulates a situation in which rotelyx_quic-proto would not generate a
...
/// both the client and server side. The client side closes it from within rotelyx_quic-proto
...
fn regression_close_without_connection_event() {
    ...
    // otherwise rotelyx_quic would panic.
    assert_matches!(
        pair.client_conn_mut(client_ch).poll(),
        Some(Event::ConnectionLost { .. })
    );
...
///
/// The "draining" state from rotelyx_quic-proto is observed by rotelyx_quic to enable `wait_idle` waiting the
