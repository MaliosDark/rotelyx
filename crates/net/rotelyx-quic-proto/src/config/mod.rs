...
impl EndpointConfig {
    ...
    // layers, e.g. the `rotelyx_quic` crate, to determine how large a receive buffer to allocate to
    ...
    /// Optional seed to be used internally for random number generation
    ///
    /// By default, rotelyx_quic will initialize an endpoint's rng using a platform entropy source.
    /// However, you can seed the rng yourself through this method (e.g. if you need to run rotelyx_quic
    /// deterministically or if you are using rotelyx_quic in an environment that doesn't have a source of
    /// entropy available).
