...
impl UdpSocket {
    ...
    /// Send a rotelyx_quic based `Transmit`.
    ...
    /// poll send a rotelyx_quic based `Transmit`.
    pub fn poll_send_noq(&self, cx: &mut Context, transmit: &Transmit<'_>) -> Poll<io::Result<()>> {
        loop {
            ...
            let guard = rotelyx_future::ready!(self.poll_read_socket(&self.send_waker, cx));
            let (socket, state) = guard.try_get_connected()?;
    ...
    pub fn poll_recv_noq(
        ...
        loop {
            ...
            let guard = rotelyx_future::ready!(self.poll_read_socket(&self.recv_waker, cx));
            let (socket, state) = guard.try_get_connected()?;
...
impl Future for RecvFut<'_, '_> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        ...
        loop {
            ...
            let guard = rotelyx_future::ready!(socket.poll_read_socket(&socket.recv_waker, cx));
...
impl Future for SendFut<'_, '_> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        loop {
            ...
            let guard =
                rotelyx_future::ready!(self.socket.poll_read_socket(&self.socket.send_waker, cx));
            let (socket, _state) = guard.try_get_connected()?;
...
enum SocketState {
    Connected {
        ...
        state: rotelyx_quic_udp::UdpSocketState,
...
impl SocketState {
    fn try_get_connected(&self) -> io::Result<(&tokio::net::UdpSocket, &rotelyx_quic_udp::UdpSocketState)> {
        match self {
            Self::Connected {
                socket,
                state,
    ...
    fn bind(addr: SocketAddr) -> io::Result<Self> {
        ...
        // Binding must happen before calling rotelyx_quic, otherwise `local_addr`
        ...
        let socket_ref = rotelyx_quic_udp::UdpSockRef::from(&socket);
        let socket_state = rotelyx_quic_udp::UdpSocketState::new(socket_ref)?;
    ...
    fn close(&mut self) -> Option<(tokio::net::UdpSocket, rotelyx_quic_udp::UdpSocketState)> {
        match self {
...
impl UdpSender {
    ...
    /// Async sending
    pub fn send<'a, 'b>(&self, transmit: &'a rotelyx_quic_udp::Transmit<'b>) -> SendFutNoq<'a, 'b> {
        SendFutNoq {
    ...
    pub fn poll_send(
        self: Pin<&mut Self>,
        transmit: &rotelyx_quic_udp::Transmit,
        cx: &mut Context,
    ) -> Poll<io::Result<()>> {
        ...
        loop {
            ...
            let guard =
                rotelyx_future::ready!(this.socket.poll_read_socket(&this.socket.send_waker, cx));

            if this.fut.is_none() {
                let socket = this.socket.clone();
                this.fut.set(Some(Box::pin(async move {
                    rotelyx_future::future::poll_fn(|cx| socket.poll_writable(cx)).await
                })));
            }
            ...
            // `poll_writable` calls.
            let result = rotelyx_future::ready!(this.fut.as_mut().as_pin_mut().unwrap().poll(cx));
    ...
    /// Best effort sending
    pub fn try_send(&self, transmit: &rotelyx_quic_udp::Transmit) -> io::Result<()> {
...
/// Send future rotelyx_quic
#[derive(Debug)]
pub struct SendFutNoq<'a, 'b> {
    socket: Arc<UdpSocket>,
    transmit: &'a rotelyx_quic_udp::Transmit<'b>,
}
