...
fn every_input_changes_the_output() {
    ...
    assert_ne!(
        *base, *other_secret,
        "the secret does not affect the output"
    );
    assert_ne!(
        *base, *other_group,
        "the group id does not affect the output"
    );
...
fn emit_vectors() {
    ...
    println!(
        "# Generated by `cargo test -p rotelyx-crypto --test pq_vectors -- --ignored emit_vectors`"
    );
