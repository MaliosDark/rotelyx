...
| Relay chaining is built and barely used | It works: `rotelyx-cli invite --through <exit>` names the far end, the caller passes their own relay, and a conversation runs through two of them with both sides agreeing a safety number. What it has not had is people: one run between two machines is not the same as a property somebody relies on, and the exposure it creates for a relay operator, dialling a host a stranger's descriptor names, has been reasoned about rather than lived with. `docs/RELAY-CHAINING-PLAN.md` records what each of the seven phases settled and what each of them got wrong first |
